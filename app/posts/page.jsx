...
export default function Posts() {
  ...
  useEffect(() => {
    const fetchUserPosts = async () => {
      try {
        if (username) {
          const response = await axios.get(`https://assignment-api-spxd.onrender.com/api/posts/${username}`);
  ...
  return (
    <main className="bg-purple-500 min-h-screen  flex justify-center">
      <div className="bg-white p-8 rounded-lg shadow-md w-[80%]  my-10 ">
        ...
        <h1 className="text-3xl font-bold mb-4 text-purple-800">Your Posts</h1>
       <Link href="/"> <FaHome className='text-violet-400' /></Link>
        </div>
